...
Original file is located at
    https://colab.research.google.com/drive/1ZYMvb85fNvQ1BrGSh4_8e7NTGH4OdC_H
"""
...
# --- DEBUGGING MODEL PATHS ---
st.sidebar.subheader("DEBUG INFO")
debug_models_dir = CONFIG["MODELS_DIR"]
st.sidebar.write(f"Looking for models directory at: {debug_models_dir}")
if os.path.exists(debug_models_dir):
    st.sidebar.success("Models directory found!")
    try:
        files_in_models = os.listdir(debug_models_dir)
        st.sidebar.write(f"Files found in {debug_models_dir}: {files_in_models}")
        if "autoencoder.keras" in files_in_models and "cnn_2lead.keras" in files_in_models and "ae_threshold.npy" in files_in_models:
            st.sidebar.success("All model files seem to be present in the directory.")
        else:
             st.sidebar.warning("One or more expected model files are missing from the models directory.")
    except Exception as e:
        st.sidebar.error(f"Error listing files in models directory: {e}")
else:
    st.sidebar.error("Models directory NOT found at this location.")

debug_ae_path = CONFIG["AUTOENC_PATH"]
debug_clf_path = CONFIG["CLF_PATH"]
debug_thr_path = CONFIG["AE_THRESHOLD_PATH"]

st.sidebar.write(f"Checking specific file paths:")
st.sidebar.write(f"- AE Model: {debug_ae_path} - Exists: {os.path.exists(debug_ae_path)}")
st.sidebar.write(f"- CNN Model: {debug_clf_path} - Exists: {os.path.exists(debug_clf_path)}")
st.sidebar.write(f"- AE Threshold: {debug_thr_path} - Exists: {os.path.exists(debug_thr_path)}")
# --- END DEBUGGING ---